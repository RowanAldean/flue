...
On Cloudflare, every agent conversation is a Durable Object with its own SQLite storage, so ownership is structural — the platform guarantees one live instance per conversation, and there is no lease protocol to operate. Execution runs on the Agents SDK's durable Tasks capability, and recovery is wake-driven:

- **Wake on start.** Whenever the Durable Object starts — after an eviction, a code deploy, or a platform reset — Flue ensures a drive run, which flags any attempt that was running when the previous instance died and reconciles it before new work starts. The SDK replays the interrupted attempt run itself on the same start, and that replay reconciles from the same evidence.
- **A durable claim.** While an attempt run is held, the SDK keeps a claim deadline in storage and wakes the object at it, so an interrupted submission recovers promptly even if no external request ever arrives. The submission's wall-clock timeout is the run's deadline: the SDK wakes at it, settles the run over an attempt that ignores its abort signal, and fences that attempt's later writes; the next drive then settles the submission through the timeout classification above. A signal-deaf attempt therefore settles at the timeout rather than after a fixed grace.
